...
6. The reviewer replies with `### 결론`, a findings table, the path of the full review it wrote to `.claude/notes/`, and a `<verdict>` tag as the reply's last line. That last line is not decoration: `.claude/hooks/record-verdict.sh` reads the reviewer's reply, not the file, so a reply that ends anywhere else was recorded as UNKNOWN and **that round was never counted** — while the file you are about to parse still carries a perfectly readable verdict. If the reply does not end with the tag, do not treat the round as spent: ask the reviewer to reply with the tag alone so the verdict reaches disk.
   ...
   Parse the verdict mechanically rather than by eye — `python3 "${HARNESS_RUN_PHASE:-scripts/harness/run_phase.py}" --parse-verdict <that file>` — then read the file for the findings you are about to dispatch. If that path is not there, read the verdict off the last line of the reviewer's reply and carry on — but treat the round as unrecorded. `record-verdict.sh` resolves the parser through the same variable, so a parser you cannot reach is one the hook could not reach either, and the budget below has almost certainly not counted this cycle. That is the case `[enforce-loop] 리뷰어 판정이 기록되지 않았습니다` describes further down: report it rather than spending another attempt against a counter that did not move. Do not ask the reviewer to paste the full review into the conversation; it is already on disk, and a review that crossed into this session once stays in its context for the rest of it.
