...
다만 무진전 감지는 **그물이 아니라 하한선**이다. 지문은 커밋, 추적 중인 변경 내용, 그리고 아직 add 하지 않은 파일들의 이름과 내용을 본다. 반대로 **어떤 파일이든 바뀌기만 하면 지문이 움직이므로** 테스트 파일 하나만 추가한 사이클은 여기 안 걸린다. "루프가 안 움직인다" 는 것만 잡아주고, phase 가 괜찮은지는 여전히 리뷰어가 판단한다.
