...
### 6. 응답 포맷도 BLUF 로 강제
LLM 의 자유서술은 결론이 본문 중간에 묻히고 scope (본 작업 / pre-existing) 가 안 갈린다. `CLAUDE.md` 에 [BLUF (Bottom Line Up Front)](https://en.wikipedia.org/wiki/BLUF_(communication)) 템플릿을 박아서 — **TL;DR → 근거(file:line) → Scope 태그 → Decision needed(추천 옵션 명시)** 4섹션을 의무로 한다. 태그 어휘 (`[NEW]/[EXISTING]/[BLOCK]/[CHANGES]/[NIT]`) 는 `reviewer`, `tester` subagent 와 통일 — 메인 세션 보고 ↔ 리뷰 결과 사이에 어휘 전환이 없게.
