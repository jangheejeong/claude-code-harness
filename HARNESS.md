...
무진전 중단은 **그물이 아니라 하한선**이다. 지문이 덮는 건 HEAD, 추적 중인 변경 내용, 그리고 아직 add 하지 않은 파일들의 이름과 내용이다 — 커밋도 add 도 안 한 새 파일 안에서만 작업해도 잡힌다. 다만 **어떤 파일이든 움직이기만 하면 지문이 바뀌므로**, 코더가 테스트 파일 하나만 추가하고 프로덕션 코드를 안 고친 사이클은 여기 안 걸린다. 이 분기가 말하는 건 "루프가 안 움직인다" 하나뿐이고, phase 가 괜찮은지에 대한 판정은 여전히 리뷰어 몫이다.
