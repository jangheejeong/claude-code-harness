...
**모델 분배 철학 (advisor + worker)**: 판단이 틀리면 라운드를 통째로 날리는 단계 = **opus** (planner, reviewer). 검색·구현·실행·기록 = **sonnet** (explorer, coder, tester, documenter). 더 똑똑한 advisor 가 계획·리뷰 품질을 올리면 worker 의 재작업이 줄어 전체 토큰이 오히려 감소한다 — 다만 그 advisor 를 어느 티어까지 올릴지는 비용 판단이고, 이 하네스는 opus 를 상한으로 둔다.
...
| 상태 | 훅의 반응 |
|---|---|
| 마지막 판정이 `BLOCK` / `REQUEST CHANGES`, 예산 남음, **작업 트리가 직전 사이클과 다름** | **exit 2** — 턴을 끝내지 못하게 막고, stderr 로 "findings 들고 coder 재투입 후 리뷰어 재실행" 을 지시. 새 BLOCK 이 기록된 뒤 오는 턴은 그 위에서 그냥 끝날 수 없다 |
| 같은 판정, 예산 남음, **작업 트리가 직전 사이클과 동일** | **exit 0** — 무진전 중단. 남은 예산을 쓰지 않는다. 같은 코드에 같은 리뷰가 한 번 더 붙을 뿐이므로 사람에게 넘긴다 |
| 같은 판정, `attempt` 가 3 도달 | **exit 0** — 턴을 끝내되 "성공이 아닙니다. 사람 개입이 필요합니다" 를 출력. 세 번 더 돌려도 못 찾을 것을 기계에 맡기지 않는다 |
| 리뷰어 판정이 **기록되지 못함** (`record_failed`) | **exit 0** — 그 사실을 한 번 알리고 표시를 소비한다. 턴은 붙잡지 않는다 (코더가 고칠 대상이 아니라 사람이 볼 사실이다). 예산 분기보다 **먼저** 판단한다 |
...
무진전 중단은 **그물이 아니라 하한선**이다. 지문이 덮는 건 HEAD, 추적 중인 변경 내용, 그리고 아직 add 하지 않은 파일들의 이름과 내용이다 — 커밋도 add 도 안 한 새 파일 안에서만 작업해도 잡힌다. 다만 **지문이 보는 파일 아무거나 하나만 움직여도 지문이 바뀌므로** (`.claude/notes` 와 gitignore 된 트리는 일부러 제외된다), 코더가 테스트 파일 하나만 추가하고 프로덕션 코드를 안 고친 사이클은 여기 안 걸린다. 이 분기가 말하는 건 "루프가 안 움직인다" 하나뿐이고, phase 가 괜찮은지에 대한 판정은 여전히 리뷰어 몫이다.

**하는 일의 범위**: 리뷰어 판정을 기록하고 재시도 상한을 강제할 뿐이다. tester 가 찾은 결함으로 도는 `/work` 안쪽 루프는 여기서 세지 않고, 코더가 4번째로 도는 것을 물리적으로 막지도 않는다 — 예산이 남았는데 멈추는 것을 막되, **직전 사이클과 작업 트리가 같으면 남은 예산을 쓰지 않고 멈춘다.** 예산이 다 되면 사람에게 넘긴다.
